  ...
  render() {
    ...
    article.innerHTML = `
            <div class="PlanImage">
                ...
                <meter value="${
                  this.stars
                }" min="0.0" max="5.0"><i class="fa-solid fa-star"></i> <span>${
      this.stars
    }</span></meter>
            </div>
            <div class="PlanInfo">
                ...
                <add-to-card data-id="${
                  this.id
                }" class="add-to-card">Add to Card</add-to-card>
                <address>
                ...
                <ol>
                    ${
                      this.place1
                        ? `<li class="active">${this.place1}</li>`
                        : '<li class="deactive"></li>'
                    }
                    ${
                      this.place2
                        ? `<li class="active">${this.place2}</li>`
                        : '<li class="deactive"></li>'
                    }
                    ${
                      this.place3
                        ? `<li class="active">${this.place3}</li>`
                        : '<li class="deactive"></li>'
                    }
                    ${
                      this.place4
                        ? `<li class="active">${this.place4}</li>`
                        : '<li class="deactive"></li>'
                    }
                    ${
                      this.place5
                        ? `<li class="active">${this.place5}</li>`
                        : '<li class="deactive"></li>'
                    }
                </ol>
                <a href="./plan.html?planName=${this.title}&tag=${this.tag}" >
                    <button class="value"><span>${
                      this.buttonText
                    }</span>-С эхэлнэ</button>
                </a>
            </div>
        `;
...
class PlanRenderer {
  ...
  filterPlacesByTag(placesData) {
    ...
    return placesData.filter(
      (place) => place.tag.toLowerCase() == this._tagFilter.toLowerCase()
    );
  }
