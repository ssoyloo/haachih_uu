...
class PlaceRenderer {
  ...
  filterPlacesBy(placesData) {
    if (
      !this._huniiTooFilter &&
      !this._oirhonGazarFilter &&
      !this._typeFilter
    ) {
      return placesData;
    }
    ...
    return placesData.filter((place) => {
      const huniiTooMatch =
        !this._huniiTooFilter ||
        place.countPeople >= parseInt(this._huniiTooFilter, 10);
      const oirhonGazarMatch =
        !this._oirhonGazarFilter ||
        place.address.includes(this._oirhonGazarFilter);
      const typeMatch =
        !this._typeFilter ||
        place.category.toLowerCase() === this._typeFilter.toLowerCase();
  ...
  filterBySide(firstPlaceList) {
    ...
    return firstPlaceList.filter((place) => {
      const starMatch =
        !this._starFilter.length ||
        this._starFilter.includes(place.stars.toString());
      const detailMatch =
        !this._detailFilter.length ||
